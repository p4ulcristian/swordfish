(ns swordfish.site.views.materialui
...
(def button (r/adapt-react-class mat/Button))
(def collapse (r/adapt-react-class mat/Collapse))
(def auto-complete (r/adapt-react-class matlab/Autocomplete))

(def input-component
  (r/reactify-component
    (fn [props] [:input (-> props (assoc :ref (:inputRef props)) (dissoc :inputRef))])))

(def reagent-text-field (r/adapt-react-class mat/TextField))
...
(def icon-button (r/adapt-react-class mat/IconButton))
(def input-adornment (r/adapt-react-class mat/InputAdornment))
(def input-label (r/adapt-react-class mat/InputLabel))
...
(def slide mat/Slide)
(def click-away-listener (r/adapt-react-class mat/ClickAwayListener))
(defn text-input [title the-keys]
  [form-control {:fullWidth true}
   [text-field {:variant  "outlined"
                :value    (let [val (db/get-in-db the-keys)]
                            (if val val ""))
                :onBlur   #(db/edit-db the-keys (clojure.string/trim (.-value (.-target %))))
                :onChange #(db/edit-db the-keys (.-value (.-target %)))
                :type     "text"
                :label    title}]])

(defn select-input [title the-keys options]
  [form-control {:fullWidth true :variant "outlined" :size "small"}
